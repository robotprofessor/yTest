...
module.exports = (wd, isIOS) => {
    ...
    async function switchWindowLast(driver) {
        let handles = await driver.windowHandles();
        return await driver.window(handles[handles.length - 1]);
    }
    async function switchWindowNext(driver) {
        let current_handle = await driver.windowHandle();
        let handles = await driver.windowHandles();
        let i = 0;
        ...
        return await driver.window(handles[i]);
    }
    async function switchWindowPre(driver) {
        let current_handle = await driver.windowHandle();
        let handles = await driver.windowHandles();
        let i = 0;
        for (i = 0; i < handles.length; i++) {
    ...
    wd.addPromiseChainMethod('switchWindow', function(opt) {
        if (opt === undefined || opt === null || opt === '') {
            return switchWindowLast(this);
        } else if (opt === 'next') {
            return switchWindowNext(this);
        } else if (opt === 'pre') {
            return switchWindowPre(this);
        } else {
            console.log('error opt: ' + opt);
        }
    });

    wd.addPromiseChainMethod('frameTo', function(frameRef) {
        if (frameRef !== null && typeof (frameRef) === 'string' && frameRef.startsWith("/")) {
            return this.elementByXPath(frameRef).then((element) => this.frame(element));
        }
        return this.frame(frameRef);
    });
};
